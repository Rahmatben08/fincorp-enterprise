...
(async () => {
  ...
  try {
    ...
    await page.waitForSelector('text=Utang & Tagihan Vendor', { state: 'visible', timeout: 10000 });
    await page.waitForTimeout(1500); // Give data time to render
    
    console.log('Taking screenshot of Manajemen Utang...');
    await page.screenshot({ path: 'payables_redesign.png', fullPage: true });

    console.log('Testing PDF Generation...');
    const downloadPromise = page.waitForEvent('download');
    await page.click('button:has-text("Cetak Daftar Utang")');
    ...
    await page.waitForSelector('text=Detail Utang Vendor', { state: 'visible', timeout: 5000 });
    await page.waitForTimeout(1000);
    console.log('Taking screenshot of Modal Lihat Detail...');
    await page.screenshot({ path: 'payables_modal.png', fullPage: true });
    ...
    if (markPaidExists > 0) {
        ...
        await page.screenshot({ path: 'payables_after_paid.png', fullPage: true });
    } else {
    ...
    await page.screenshot({ path: 'error_payables.png', fullPage: true });
  } finally {
    await browser.close();
  }
})();
