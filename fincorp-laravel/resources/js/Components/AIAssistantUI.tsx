...
const AIAssistantUI: React.FC = () => {
  ...
  const sendMessage = async (e: React.FormEvent) => {
    ...
    try {
      // Fitur ini sedang dalam pemeliharaan (migrasi ke arsitektur Laravel)
      // TODO: Implementasikan integrasi OpenAI/Claude API langsung dari backend Laravel
      // const response = await axios.post('/api/ai/chat', { query: userMessage });
      await new Promise(resolve => setTimeout(resolve, 1000)); // Simulasi loading
      throw new Error("Maintenance");
    } catch (error) {
      setMessages(prev => [...prev, { sender: 'ai', text: 'Maaf, fitur AI Assistant sedang dalam pemeliharaan (migrasi arsitektur). Silakan coba lagi nanti.' }]);
    } finally {
      setLoading(false);
    }
  };
